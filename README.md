# TDX Sync Rust

通达信协议解析的 Rust 实现，用于获取股票信息。

## 功能特性

- ✅ 完整的协议帧格式支持（请求帧和响应帧）
- ✅ 数据编码/解码工具（小端序、变长整数、价格编码等）
- ✅ TCP 客户端连接管理
- ✅ 多种连接方式（单个地址、多个地址轮询/随机/遍历）
- ✅ 多种消息类型支持：
  - 建立连接
  - 心跳包
  - 获取股票数量
  - 获取股票代码列表
  - 行情信息（5档报价）
  - K线数据（部分实现）
  - 分时数据（部分实现）
  - 分时成交（部分实现）

## 快速开始

### 基本使用

```rust
use tdx_sync_rust::*;

fn main() -> Result<(), ClientError> {
    // 连接到服务器
    let client = dial("124.71.187.122")?;
    
    // 获取股票数量
    let count = client.get_count(Exchange::SH)?;
    println!("上海交易所股票数量: {}", count);
    
    // 获取股票代码列表
    let codes = client.get_code(Exchange::SH, 0)?;
    println!("获取到 {} 只股票", codes.codes.len());
    
    // 获取行情信息
    let quotes = client.get_quote(&["sz000001".to_string(), "sh600000".to_string()])?;
    for quote in quotes {
        println!("股票: {}{}, 收盘价: {:.2}元", 
            quote.exchange.as_str(), quote.code, quote.k.close.to_yuan());
    }
    
    Ok(())
}
```

### 连接方式

```rust
// 方式1: 连接到指定地址
let client = dial("124.71.187.122")?;

// 方式2: 遍历多个地址连接（推荐，自动重试）
let client = dial_hosts_range(&["124.71.187.122", "122.51.120.217"])?;

// 方式3: 随机选择一个地址连接
let client = dial_hosts_random(&["124.71.187.122", "122.51.120.217"])?;

// 方式4: 使用默认连接方式（遍历默认服务器列表）
let client = dial_default()?;
```

### 测试服务器连接速度

```rust
use tdx_sync_rust::*;

let hosts = &["124.71.187.122", "122.51.120.217", "111.229.247.189"];
let results = fast_hosts(hosts);

for result in results {
    println!("{} - {:.2}ms", result.host, result.duration.as_secs_f64() * 1000.0);
}
```

## 运行示例

```bash
# 基本使用示例
cargo run --example basic

# 测试服务器连接速度
cargo run --example fast_hosts
```

## 项目结构

```
tdx-sync-rust/
├── src/
│   ├── lib.rs              # 库入口
│   ├── client.rs           # TCP 客户端实现
│   ├── dial.rs             # 连接管理
│   └── protocol/
│       ├── mod.rs          # 协议模块入口
│       ├── constants.rs    # 常量定义
│       ├── frame.rs        # 帧格式编解码
│       ├── types.rs        # 数据类型定义
│       ├── codec.rs        # 编码/解码工具函数
│       └── messages.rs     # 各种消息类型的编解码
├── examples/
│   ├── basic.rs            # 基本使用示例
│   └── fast_hosts.rs      # 测试服务器连接速度示例
├── tests/
│   └── protocol_test.rs    # 协议测试
└── tdx-test/               # 测试数据
```

## 依赖

- `bytes` - 字节操作
- `flate2` - zlib 压缩/解压
- `encoding_rs` - GBK/UTF-8 编码转换
- `thiserror` - 错误处理
- `tokio` - 异步运行时（部分功能）
- `rand` - 随机数生成
- `serde` / `serde_json` - JSON 序列化（测试数据）

## 开发状态

当前实现了：
- ✅ 基础协议框架
- ✅ TCP 客户端连接
- ✅ 基本消息类型（连接、心跳、数量、代码、行情）
- ✅ 连接管理（多种连接方式）
- ✅ 测试数据验证

正在开发：
- 🔄 完整的行情解析（5档报价、成交量等）
- 🔄 K线数据完整实现
- 🔄 分时数据和成交数据
- 🔄 异步客户端支持
- 🔄 连接池管理

## 参考

- [协议文档](./tdx-protocol.md) - 完整的协议文档
- [测试数据](./tdx-test/) - 测试数据和验证工具
- [injoyai/tdx](https://github.com/injoyai/tdx) - Go 语言参考实现

## License

MIT
