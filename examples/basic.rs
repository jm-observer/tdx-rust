...
async fn main() -> Result<(), ClientError> {
    ...
    // 获取股票数量
    println!("2. 获取股票数量...");
    let count_sh = client.get_count(Exchange::SH).await?;
    let count_sz = client.get_count(Exchange::SZ).await?;
    println!("   上海: {} 只, 深圳: {} 只\n", count_sh, count_sz);

    // 获取股票代码列表
    println!("3. 获取股票代码列表...");
    let codes = client.get_code(Exchange::SH, 0).await?;
    ...
    let quotes = client
        .get_quote(&["sz000001".to_string(), "sh600000".to_string()])
        .await?;
    for quote in &quotes {
        println!("   {:?}", quote);
    }
    ...
    // 获取日K线数据
    println!("5. 获取日K线数据...");
    let klines = client.get_kline_day("sz000001", 0, 5).await?;
    ...
    // 获取分时数据
    println!("6. 获取分时数据...");
    let minute = client.get_minute("sz000001").await?;
    ...
    // 获取集合竞价数据
    println!("7. 获取集合竞价数据...");
    let mut auction = client.get_call_auction("sz000001").await?;
    ...
    let gbbq = client.get_gbbq("sz000001").await?;
